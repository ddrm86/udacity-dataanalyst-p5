...
if __name__ == "__main__":
    ...
    features_list = ['poi', 'salary', 'to_messages', 'deferral_payments', 'total_payments', 'exercised_stock_options',
                     ...
                     'director_fees', 'deferred_income', 'long_term_incentive', 'from_poi_to_this_person', 'stocks_vs_payments', 'salary_vs_payments']
    ...
    feature_reduction = FeatureUnion([('pca', pca), ('fss', fss)])
    ...
    pipe = Pipeline(steps=[('feature_reduction', feature_reduction), ('clf', clf)])
    ...
    param_grid = [
      {'clf__min_samples_split': [2, 5, 10, 15, 20, 50], 'clf__criterion': ['gini', 'entropy'],
       ...
       'clf__max_depth': [3, 4, 5, 6, 7, 8, 9, 10, 20, 50], 'feature_reduction__pca__n_components': range(1, len(features_list)),
       'feature_reduction__fss__k': range(1, len(features_list))}
     ]
